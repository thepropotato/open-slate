...
import { asDestination } from './destination'
 ...
 * address, then a plain search run by the browser's own default engine.
 */
export function SearchBar() {
  ...
  const askWeb = search.webSuggestions && !maths && !destination && query.length > 1
  const debounced = useDebounced(askWeb ? query : '', 150)
  const web =
    useAsyncValue(debounced ? `w:${debounced}` : null, () =>
      remoteSuggest(debounced, { limit: 5, openIn: tiles.openIn }),
    ) ?? []

  // Local hits first: the web list only ever fills the space below them.
  const suggestions = [...local, ...web].slice(0, 8)
  // One rule where your own things end and the web's guesses begin.
  ...
  const submit = () => {
    ...
    if (!query) return
    if (!searchDefault(query, tiles.openIn)) {
      // Only outside the extension, where there is no browser default to reach.
      openUrl(`https://www.google.com/search?q=${encodeURIComponent(query)}`, tiles.openIn)
    }
  }
  ...
  const placeholder = search.placeholder || 'Search the web'
